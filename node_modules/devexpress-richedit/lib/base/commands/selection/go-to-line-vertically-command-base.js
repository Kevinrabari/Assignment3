...
exports.GoToLineVerticallyCommandBase = void 0;
...
var GoToLineVerticallyCommandBase = (function (_super) {
    ...
    GoToLineVerticallyCommandBase.prototype.extendTableCellsSelection = function () {
        var currentLayoutPosition = this.getInitialLayoutPosition();
        if (!currentLayoutPosition)
            return false;
        var cellIterator = new table_cell_iterator_1.TableCellIterator(currentLayoutPosition, this.control.layout, this.control.measurer);
        var selectedCells = this.getSelectedCells();
        var areCellsSelectedInSeries = this.selection.tableInfo.extendedData.areCellsSelectedInSeries;
        if (this.extendSelection() && (selectedCells.length > 1 || this.canGoToNextTableRow(cellIterator)) && areCellsSelectedInSeries) {
            var isForward = this.selection.isCollapsed() ? this.isForwardDirection() : this.selection.forwardDirection;
    ...
    GoToLineVerticallyCommandBase.prototype.executeCore = function (_state, _options) {
        ...
        var initialLayoutPosition = this.getInitialLayoutPosition();
    ...
    GoToLineVerticallyCommandBase.prototype.getInitialLayoutPosition = function () {
        ...
        return (subDocument.isMain()
            ...
            .create(new layout_position_creator_1.LayoutPositionCreatorConflictFlags().setDefault(useBoxEndPos), new layout_position_creator_1.LayoutPositionCreatorConflictFlags().setDefault(false));
    };
    GoToLineVerticallyCommandBase.prototype.canGoToNextTableRow = function (_cellInterator) {
        return false;
    };
    ...
    GoToLineVerticallyCommandBase.prototype.performTableCellsSelection = function (isForward, nextRow, firstCell, lastCell) {
        var lastSelectedCell = isForward ? lastCell : firstCell;
        var lastCellIndex = table_utils_1.TableCellUtils.getCellIndexByColumnIndex(nextRow, table_utils_1.TableCellUtils.getStartColumnIndex(lastSelectedCell));
        var newFirstCell = isForward ? firstCell : lastCell;
        var newLastCell = nextRow.cells[lastCellIndex];
        if (newFirstCell === newLastCell) {
            var firstPos_1 = newFirstCell.startParagraphPosition.value;
            var lastPos_1 = newFirstCell.endParagrapPosition.value - 1;
            this.selection.changeState(function (state) { return state.setInterval(fixed_1.FixedInterval.fromPositions(firstPos_1, lastPos_1)).setForwardDirection(isForward); });
            return true;
        }
        else
    ...
    GoToLineVerticallyCommandBase.prototype.isForwardDirection = function () {
        throw new Error(errors_1.Errors.NotImplemented);
    };
